# Manufacturing Process Analysis & Predictive Maintenance Analytics

## Project Overview

This project focuses on analyzing manufacturing process and machine operational data to identify inefficiencies, operational gaps, abnormal patterns, and machine failure conditions. The analysis was performed using Python, SQL, and Power BI to support data-driven decision-making and improve manufacturing reliability.

The project combines statistical analysis, anomaly detection, operational monitoring, and business intelligence techniques to evaluate machine performance and optimize manufacturing processes.

---

# Objectives

- Analyze manufacturing and machine operational data
- Identify machine failure patterns and operational inefficiencies
- Perform statistical and exploratory data analysis
- Detect anomalies and abnormal operational conditions
- Evaluate defect and failure trends using SQL
- Generate business insights for process optimization
- Develop interactive Power BI dashboards for operational monitoring

---

# Technologies Used

- Python
- Pandas
- NumPy
- Matplotlib
- Seaborn
- SQL
- Power BI
- Jupyter Notebook

---

# Dataset Information

Dataset Used:
AI4I 2020 Predictive Maintenance Dataset

Dataset Source:
https://www.kaggle.com/datasets/stephanmatzka/predictive-maintenance-dataset-ai4i-2020

---

# Project Workflow

```text
Data Collection
       ↓
Data Cleaning & Preprocessing
       ↓
Exploratory Data Analysis (EDA)
       ↓
Statistical Analysis
       ↓
Anomaly Detection
       ↓
SQL-Based Operational Analysis
       ↓
Business Insights Generation
       ↓
Power BI Dashboard Development
```

---

# Python Analysis Performed

- Data cleaning and preprocessing
- Missing value and duplicate analysis
- Statistical analysis
- Correlation analysis
- Machine failure analysis
- Tool wear analysis
- Temperature analysis
- Torque analysis
- Anomaly detection using IQR
- Feature engineering
- Operational gap analysis
- Failure type analysis

---

# SQL Analysis Performed

- Failure rate analysis
- Product-wise failure analysis
- Operational condition analysis
- High tool wear analysis
- Failure category analysis
- Temperature-based operational analysis
- Anomaly identification queries
- Performance comparison analysis

---

# Key Insights

- Machines with higher tool wear showed increased failure probability.
- Process temperature fluctuations influenced machine instability.
- Torque outliers indicated abnormal operational conditions.
- Certain product categories experienced more operational failures.
- Heat dissipation and overstrain failures occurred more frequently.

---

# Business Recommendations

- Implement predictive maintenance strategies
- Monitor torque and temperature thresholds
- Optimize operational load conditions
- Schedule preventive maintenance for high wear machines
- Improve operational monitoring for high-risk product categories

---

# Power BI Dashboard Features

- Failure monitoring dashboard
- Operational KPI tracking
- Machine performance analysis
- Failure trend visualization
- Interactive operational filtering
- Reliability monitoring visuals

---

# Project Outcome

The project successfully identified operational inefficiencies, abnormal patterns, and failure-prone conditions affecting manufacturing reliability. The analysis demonstrates how data analytics techniques can support predictive maintenance and improve process performance in manufacturing environments.

---

# Author

Anand Pandey
